...
from adj_importer import adj_importer, to_mininet
...
def display_link_utilization(network : nx.Graph):
    ...
    for u,v,data in network.edges(data=True):
        ...
        #print(data["used_bandwidth"])
        print(data["used_bandwidth"] / data["bandwidth"])
...
if __name__ == "__main__":
    ...
    while timestamp < 333:
        ...
        #while len(D) < 20:
        for nodes in range(3, 24, 3):

            nodes = list(range(nodes))

            network_graph = adj_importer(adj)#, rename=False)
            new_network_graph = nx.Graph()

            for l in range(len(adj)):
               u = adj[l][0]
               v = adj[l][1]
               new_network_graph.add_edge(f"s{u}", f"s{v}", bandwidth = cap[l], used_bandwidth =  0, power = 0.42, active = False)

            topo = to_mininet(new_network_graph)

            #topo = networkx_to_mininet(network_graph)
            network_graph = topo.convertTo(nx.MultiGraph)
            network_graph = nx.Graph(network_graph)
            ...
            D = load_big_demand("data/H.txt", timestamp, nodes)  
            print(len(D))
            #print(D)
            ...
            for l in nx.node_link_data(network_graph)["links"]:
                #print(new_network_graph[l["source"]][l["target"]])
                new_link_data[l["source"], l["target"]] = {"interface": link_interface(l), "bandwidth": new_network_graph[l["source"]][l["target"]]["bandwidth"], "used_bandwidth": 0, "power": 0.42, "active": False}
                #new_link_data[l["source"], l["target"]] = {"interface": link_interface(l), "bandwidth": 100, "used_bandwidth": 0, "power": 0.42, "active": False}


            nx.set_edge_attributes(network_graph, new_link_data)

            #print(nx.node_link_data(network_graph)["links"])
            ...
            print(len(nodes))
            #display_link_utilization(network_graph)
        ...
        with open('results/geant/resdn_result_optimized.txt', 'a') as f:
            for i in resdn_consumptions:
                print(i, end="\t", file=f)
            print(" ", file=f)


        with open('results/geant/resdn_times.txt', 'a') as f:
            for i in resdn_times:
                print(i, end="\t", file=f)
            print(" ", file=f)


        with open('results/geant/minpow_result.txt', 'a') as f:
            for i in minpow_consumptions:
                print(i, end="\t", file=f)
            print(" ", file=f)


        with open('results/geant/minpow_times.txt', 'a') as f:
            for i in minpow_times:
                print(i, end="\t", file=f)
            print(" ", file=f)


    # with open('resdn_result.txt', 'a') as f:
    #     print("\n", file=f)

    # with open('minpow_result.txt', 'a') as f:
    #     print("\n", file=f)
